...
from lib.auth.db.postgres import get_user_db
from lib.auth.model.schemas import ChangePassword, UserUpdate
from lib.auth.repository.redis_repo import RedisRepository
...
class UserManager(IntegerIDMixin, BaseUserManager[User, int]):
    ...
    async def create(
        ...
        user_dict["is_superuser"], user_dict["is_verified"], user_dict["role_id"] = False, False, 1
    ...
    async def send_email_verify_code(email: str, redis: aioredis.Redis, logger):
        ...
        await RedisRepository.add_email_verify_code(email, code, redis)
    ...
    async def check_email_verify_code(email, verify_code):
        ...
        code_in_redis = await RedisRepository.get_email_verify_code(email, redis)
